...
{
    public class FileSystemBFS10 : FileSystemBFSBase
    {
        public FileSystemBFS10()
        {
            EntrySize = 0x20;
            FatAddress = 0x600;
            m_RootAddress = 0x400;
        ...
        public void FormatDisk(Disk disk)
        {
            ...
            if (Program.m_Admin)
            {
                ...
                newOffset = WriteToDisk("BFS10".PadRight(6, ' '), newOffset);
        ...
        public void CreateDirectory(string path, bool IsProtected = false, bool IsHidden = false)
        {
            ...
            if (m_Disk.m_FileSystemFormat == FileSystemType.BFS10)
            {
                if (name.Length > NameLength + FiletypeLength)
                {
                    Console.WriteLine("entry name is to long for BFS10");
                    return;
                }
            }
            int offsetDir;
            ...
            createEntryBFS10(name, 1, offsetDir, ref EntryConut, true, IsProtected, IsHidden);
        ...
        public void CreateFile(string path, int size, bool IsProtected = false, bool IsHidden = false)
        {
            ...
            createEntryBFS10(name, (ushort)size, offsetDir, ref EntryConut, false, IsProtected, IsHidden);
            if (!string.IsNullOrEmpty(parentDirectory))
            {
        ...
        public void BFS10LoadFile(Disk disk)
        {
            if (disk == null)
            {
                return;
        ...
        void createEntryBFS10(string name, ushort sizeInSectors, int offsetDir, ref int EntryConut, bool IsDirectory = false, bool IsProtected = false, bool IsHidden = false)
        {
            name = name.PadRight(NameLength + FiletypeLength, '\0');
            Console.WriteLine($"Creating {name} entry");

            ushort page = BFS10findFreeSector(sizeInSectors);
            ...
            if (FileType != "")
            {
                offset = WriteToDisk(FileName.PadRight(NameLength, '\0'), offset);
                offset = WriteToDisk(FileType.PadRight(FiletypeLength, '\0'), offset);
            }
            else
            {
                offset = WriteToDisk(FileName.PadRight(NameLength + FiletypeLength, '\0'), offset);
            }
        ...
        public ushort BFS10findFreeSector(ushort sizeInSectors = 1)
        {
            int result = -1;
