...
namespace BCG16CPUEmulator
{
    public class ArgumentFunctions : CPUSettings
    {
        ...
        public static void SetDriveBinary(string[] args, ref int i)
        {
            ...
            i++;
            string path = args[i];
            decodeSettings(setting, out int index, out bool Use80, out bool writeEnable, out FileSystemType fileSystemType, out DiskSize diskSize);
            ...
            Disk disk = new Disk()
            {
                ...
                m_Is80Track = Use80,
                m_WriteEnable = (byte)(writeEnable ? 1 : 0)
            };

            m_DiskPaths.Add(index, disk);
        }

        static void decodeSettings(string setting, out int index, out bool Use80Tracks, out bool writeEnable, out FileSystemType fileSystemType, out DiskSize diskSize)
        {
            ...
            for (int i = 0; i < settings.Length; i++)
            {
                ...
                else if (settings[i].StartsWith("F", StringComparison.OrdinalIgnoreCase))
                {
                    diskSize = DiskSize._F3MB;
                }
                else if (settings[i].StartsWith("H20", StringComparison.OrdinalIgnoreCase))
                {
                    diskSize = DiskSize._20MB;
                }
            }
        }
    }
